<?php
session_start();
include 'includes/db.php'; // $pdo défini ici
...
$stmtTerr = $pdo->query("SELECT id, nom FROM terrains");
$terrains = $stmtTerr->fetchAll(PDO::FETCH_ASSOC);

// Filtres GET
$filtre_jour    = $_GET['filtre_jour']   ?? '';
$filtre_mois    = $_GET['filtre_mois']   ?? '';
$filtre_terrain = $_GET['terrain_id']    ?? '';
...
if ($filtre_terrain) {
    $where .= " AND r.id_terrain = ?";
    $params[] = $filtre_terrain;
}

// Stat global commission admin
...
// Graphiques commissions par mois/jour
$stmtGraph = $pdo->prepare("
    SELECT DATE_FORMAT(r.date, '%Y-%m') AS mois, SUM(rev.commission_admin) AS total
    FROM reservations r
    JOIN revenus rev ON r.id = rev.id_reservation
    WHERE $where
...
$labels = array_column($revenusMois, 'mois');
$values = array_column($revenusMois, 'total');

$labelsJours = $valuesJours = [];
if (!empty($labels)) {
    $dernier = end($labels);
    $stmtDay = $pdo->prepare("
        SELECT DATE(r.date) AS jour, SUM(rev.commission_admin) AS total
        FROM reservations r
        JOIN revenus rev ON r.id = rev.id_reservation
        WHERE DATE_FORMAT(r.date, '%Y-%m') = ?
        GROUP BY jour ORDER BY jour
    ");
    $stmtDay->execute([$dernier]);
    $jours = $stmtDay->fetchAll(PDO::FETCH_ASSOC);
    $labelsJours = array_column($jours, 'jour');
    $valuesJours = array_column($jours, 'total');
}
...
foreach ($terrains as $t) {
    $stmt = $pdo->prepare("
        SELECT COUNT(r.id) AS total_res, SUM(rev.commission_admin) AS commission
        FROM reservations r
        JOIN revenus rev ON r.id = rev.id_reservation
        WHERE r.id_terrain = ?
    ");
    ...
    $statsTerrains[] = [
        'id' => $t['id'],
        'nom' => $t['nom'],
        'total_res' => $res['total_res'] ?? 0,
        'commission' => $res['commission']   ?? 0
    ];
}

// Récupérer propriétaires/joueurs si nécessaire
$stmtP = $pdo->query("SELECT id, nom, email FROM users WHERE type = 'proprietaire'");
$proprietaires = $stmtP->fetchAll(PDO::FETCH_ASSOC);
$stmtJ = $pdo->query("SELECT id, nom, email FROM users WHERE type = 'joueur'");
...
<select name="terrain_id" class="form-control">
  ...
  <div class="container py-4">
        ...
        <form method="get" class="mb-4 form-filtre-stats">
            <div class="row g-2 align-items-end">
                ...
                <div class="col-auto">
                    ...
                    <a href="dashboard_admin.php" class="btn btn-secondary">Réinitialiser</a>
                </div>
            </div>
        </form>
        <div class="row mb-4">
            ...
            <div class="col-md-3">
                <div class="card text-center shadow-sm">
                    <div class="card-body">
                        <h5 class="card-title">Revenu Propriétaires</h5>
        ...
        <h3 class="mt-5">Liste des propriétaires</h3>
        ...
        <h3 class="mt-5">Liste des joueurs</h3>
        <div class="table-responsive">
            <table class="table table-bordered align-middle">
                <thead>
                    <tr>
...
<table class="table">
    <thead><tr><th>Terrain</th><th>Réservations</th><th>Commission Admin (DT)</th></tr></thead>
    <tbody>
    <?php foreach ($statsTerrains as $stat): ?>
    <tr>
        <td><?= htmlspecialchars($stat['nom']) ?></td>
        <td><?= $stat['total_res'] ?></td>
        <td><?= number_format($stat['commission'],2) ?></td>
    </tr>
    <?php endforeach; ?>
    </tbody>
</table>
